...
class Tickbox {
	...
	public function __construct($isEditorButton = false, $isLoadedMediaData = false) {
		...
		if(!$this->isLoadedMediaData) {
			...
			add_action('admin_enqueue_scripts', 'readMoreMebiaScripts' );
		}
	}

	public function readMoreMebiaScripts()
	{
	...
	private function mediaButton() {
		...
		if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) && $typenow != 'download' ) {
			...
			$output = '<a href="javascript:void(0);" class="button yrm-thickbox" style="padding-left: .4em;">' . $img . __('Countdown Builder', YRM_LANG) . '</a>';
		...
		if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) && $typenow != 'download' ) { ?>
			<script type="text/javascript">
				function insertCountdownDownload() {
					...
					var moreAttribute = '';
					var lessAttribute = '';
					if (jQuery('#yrm-shortcode-more-attribute-enable').is(':checked')) {
						moreAttribute = 'more="'+jQuery('#yrm-shortcode-more-attribute').val()+'"';
					}
					if (jQuery('#yrm-shortcode-less-attribute-enable').is(':checked')) {
						lessAttribute = 'less="'+jQuery('#yrm-shortcode-less-attribute').val()+'"';
					}

					// Send the shortcode to the editor
					window.send_to_editor('[expander_maker id="'+id+'" '+moreAttribute+' '+lessAttribute+']'+selection+'[/expander_maker]');
					jQuery('#yrm-dialog').dialog('close')
				}
			...
			<div id="yrm-dialog" style="display: none;">
				<div class="wrap" style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">
					...
					<?php endif; ?>
					<p class="submit">
						<input type="button" id="edd-insert-download" class="button-primary" value="<?php _e('Insert', YRM_LANG)?>" onclick="insertCountdownDownload();" />
				...
				<style>
					.yrm-custom-more-wrapper, .yrm-custom-less-wrapper {padding-top: 5px}
					.yrm-custom-more-label-wrapper {margin-bottom: 12px}
					.yrm-custom-more-hide,.yrm-custom-less-hide  {display: none;}
					#yrm-shortcode-more-attribute-enable:checked ~ .yrm-custom-more-hide,
					#yrm-shortcode-less-attribute-enable:checked ~ .yrm-custom-less-hide {display: block;}
				</style>
			</div>
			<?php
		}
